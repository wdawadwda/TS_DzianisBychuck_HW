...
class School implements SchoolInterface {
  ...
  private isSubjectsValid = (subject: Subjects): subject is Subjects => {
  ...
  public getStudentsAndTeachers = () => {
    if (this.isSubjectsValid(this.subjects)) {
      return Object.values(this.subjects).reduce(
        (accumulator, current) => {
          accumulator.students += current.students;
          accumulator.teachers += current.teachers;
          return accumulator;
        },
  ...
  public getStudentCount = () => {
    return Object.values(this.subjects).reduce((accumulator, current) => {
      return (accumulator +=
        current.students / Object.keys(this.subjects).length);
    }, 0);
  };
  ...
  public getSortedSubjects = () => {
    return this.subjectsArray.sort(
      (a, b) => b[Object.keys(b)[0]].teachers - a[Object.keys(a)[0]].teachers
    );
  };
}
