...
function uniqueMoviesGenre(movies: Movies): ArrayString {
...
function uniqueActors(movies: Movies): ArrayString {
  const uniqueActors = new Set(movies.flatMap((movie) => movie.actors));
  return [...uniqueActors];
}
...
// Task 5
//___________________________________________________________________________________
function SearchByYear(movies: Movie[], year = 0): Movies | string {
...
function SearchByTitle(movies: Movie[], search = ''): Movies | string {
  if (search) {
...
function SearchByTitleAndPlot(movies: Movies, search = ''): Movies | string {
  if (search) {
...
function filterMoviesByField<T extends string | number | string[]>(
  movies: Movies,
...
const filteredMovies = filterMoviesByField(
  otherMovies,
  'title',
  'Harry Potter'
);
